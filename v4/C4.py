...
RADIUS = int(SQUARE_SIZE/2-5) #circles (pieces) on board

screen = pygame.display.set_mode(size)  #set pygame screen size 
draw_board(board)                       #draw gameboard
pygame.display.update()                 #needed for pygame to update display
...
while not game_over:        #gameplay
    ...
    for event in pygame.event.get():    #for closing screen x
        ...
        if event.type==pygame.MOUSEBUTTONDOWN:      #when mouse clicked
            ...
            else: turn+=1                           #fixed issue with changing player turn when off top of board
